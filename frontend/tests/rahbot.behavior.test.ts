...
interface TestCase {
  ...
  expectedFormOpen: boolean;
  expectedCTAType: string;
}
...
TEST_CASES.forEach((tc, idx) => {
  ...
  if (tc.expectedServiceId === null) {
    servicePass = resolved.service === null;
  } else if (tc.expectedServiceId === undefined) {
    servicePass = true;
  } else {
    servicePass = resolved.service?.id === tc.expectedServiceId;
  }
...
const res1 = resolveService('Tell me about ERP', ctx);
const dec1 = buildBotDecision('Tell me about ERP', res1, ctx);
...
const res2 = resolveService('What about pricing?', ctx);
const dec2 = buildBotDecision('What about pricing?', res2, ctx);
...
const res3 = resolveService('Actually tell me about internships', ctx);
const dec3 = buildBotDecision('Actually tell me about internships', res3, ctx);
...
// Step 4: Follow up on internships "How can students apply?"
const res4 = resolveService('How can students apply?', ctx);
const dec4 = buildBotDecision('How can students apply?', res4, ctx);
const step4Pass = res4.service?.id === 'program-internship' && dec4.intent === 'internship';
console.log(`Context Step 4 ("How can students apply?" inherits internship): ${step4Pass ? '✅ PASS' : '❌ FAIL'}`);

const totalTests = TEST_CASES.length + 4;
const totalPassed = passedCount + (step1Pass ? 1 : 0) + (step2Pass ? 1 : 0) + (step3Pass ? 1 : 0) + (step4Pass ? 1 : 0);
