    ...
    <div className="space-y-6 font-sans">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-6 border-b border-slate-800">
        ...
        <button
          ...
          disabled={isRunning}
          className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white text-xs font-semibold rounded-xl shadow-lg shadow-blue-600/25 transition-all"
        >
          {isRunning ? (
            <>
      ...
      {runResult && (
        <div className="p-4 bg-emerald-500/10 border border-emerald-500/30 rounded-2xl flex items-start justify-between text-xs text-emerald-300">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-bold text-white">Pipeline Execution Succeeded!</p>
              <p className="text-slate-300 mt-0.5">{runResult.message}</p>
              {runResult.post && (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
          ...
          <div className="space-y-2 text-xs text-slate-300">
            <div className="flex justify-between py-1.5 border-b border-slate-800">
              ...
              <span className="font-mono font-semibold text-white">18:00 IST (Daily)</span>
            </div>
            <div className="flex justify-between py-1.5 border-b border-slate-800">
              <span className="text-slate-500">Topic Rotation Engine</span>
              <span className="text-slate-200">Enabled (Anti-Duplication Matrix)</span>
            </div>
            <div className="flex justify-between py-1.5 border-b border-slate-800">
              <span className="text-slate-500">Primary Domain Focus</span>
              <span className="text-slate-200">ERP, SaaS, Cloud, Microservices</span>
            </div>
            <div className="flex justify-between py-1.5">
              <span className="text-slate-500">Target Word Count</span>
              <span className="text-slate-200">1,200 – 1,800 Words</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4">
          ...
          <div className="pt-2 flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-white">Direct Auto-Publish</p>
                ...
                data.runs.map((run: any) => (
                  <tr key={run.id} className="hover:bg-slate-800/30">
                    ...
                    <td className="py-3 px-4">
                      <span className="font-medium text-slate-200">{run.topic}</span>
                    </td>
